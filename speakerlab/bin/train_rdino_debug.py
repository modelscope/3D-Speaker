...
import os
import sys
sys.path.append('/mnt/workspace/code/3D-Speaker')
os.chdir('/mnt/workspace/code/3D-Speaker/egs/sv-rdino/voxceleb')
import json
import math
import numpy
import argparse
...
def main():
    args_list = ['--config', 'conf/rdino.yaml', '--gpu', '4',
    '--data', 'data/vox2_dev/wav.scp', '--noise', 'data/musan/wav.scp', '--reverb','data/rirs/wav.scp', '--exp_dir','exp/rdino']
...
def train_one_epoch(student, teacher, dino_loss, regularized_loss, train_loader, optimizer, lr_schedule, \
    ...
    for it, data in enumerate(
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            fbank_dim = data.shape[2]
            frame_dim = data.shape[3]
            ...
            student_output = torch.cat((student_global_output, student_local_output), axis=0) # torch.Size([312, 65536])
            ...
            later_lambd = numpy.linspace(config.lambd, config.lambd, 50)
