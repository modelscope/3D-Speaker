...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  torchrun --nproc_per_node=$num_gpu --master_port=29502 speakerlab/bin/train.py --config conf/eres2net.yaml --gpu $gpus \
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  nj=12
  torchrun --nproc_per_node=$nj --master_port=29502 speakerlab/bin/extract.py --exp_dir $exp_dir \
