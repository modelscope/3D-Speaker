...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  torchrun --nproc_per_node=$num_gpu speakerlab/bin/train.py --config conf/resnet.yaml --gpu $gpus \
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  torchrun --nproc_per_node=12 speakerlab/bin/extract.py --exp_dir $exp_dir \
