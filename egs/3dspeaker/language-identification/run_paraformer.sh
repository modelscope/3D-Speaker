...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  torchrun --nproc_per_node=$num_gpu speakerlab/bin/train_para.py --config conf/eres2net_para.yaml --gpu $gpus \
           --data $data/mejck/train/new_train_mejck_70w.csv --noise $data/musan/wav.scp --reverb $data/rirs/wav.scp --exp_dir $exp_dir
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  torchrun --nproc_per_node=16 local/predict_para.py --exp_dir $exp_dir \
           --data $data/mejck/test/wav.scp --use_gpu --gpu $gpus
fi
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  ...
  python local/compute_acc.py --predict $exp_dir/results/predict.txt --ground_truth $data/mejck/test/utt2spk \
